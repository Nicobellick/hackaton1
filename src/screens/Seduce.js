...
const tatooine = ['- Prends ta douche au vinaigre', '- Rince toi le visage avec du salami', '- Sentir le white spirit', '- Lui/la/leurs/non-binaire faire rire'];
const corellia = ['- Ne te rase pas', '- Carresser dans le sens du poil', '- Fais des dreadlocks', '- Origine Portuguaise appréciée']
const ryloth = ['- Montrer son gros calibre', '- Eviter de tirer à blanc', '- Avoir la fleur au bout du fusil', '- Savoir mettre une bonne cartouche']
const naboo = ['- Ne pas déboulonner trop vite', '- Ne pas avoir de vice', '- Etre à la Wild', '- Quentin Kaiser de quentin']
        ...
        <div className='bgSeduce'>
            ...
            <ul className='listPlanet'>
                ...
                {document.querySelector('.advice').classList.add('visible');
                    setAdvice('ryloth');
                    filtreArray(sliceWars);
                    console.log(arrayStar)   }}>Ryloth</li>
                <li className='planet' onClick={() => 
                {document.querySelector('.advice').classList.add('visible');
                    setAdvice('naboo');
                    filtreArray(sliceWars);
                    console.log(arrayStar)   }}>Naboo</li>
            </ul>
            <div>
                <div className='advice'>
                    <div classList='listAdvice' style={{width: '400px'}}>
                        <h3>Conseil pour les chemins boueux</h3>
                        {advice == 'tatooine' ? 
                        <ul>
                        ...
                        <li className='conseilList'>{corellia[3]}</li>
                         ...
                         : null }
                    {arrayStar == '' ? null : arrayStar.map(result => 
                    

                        <img src={result.image} key={result.born} className='imgExemple' alt={result.name}/>
