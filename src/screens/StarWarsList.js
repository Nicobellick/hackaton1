...
class StarWarsList extends React.Component {
    ...
    return(
        <div className="loveRoomPlanet">
            ...
            <Link to={{pathname:'/loveroom', data:{characterLover: this.state.characterLover, characterLoverImg: this.state.characterLoverImg}}}>Rentrer dans la Love Room</Link> 
            <div className="starWarsLoveurs">
            {sliceArray.map((character) => (
                    <div className="globalCardLoveur" key={character.name} onClick={() => 
                    (
                        this.setState({characterLover: character.name}),
                        this.setState({characterLoverImg: character.image}),
                        console.log(this.state.characterLover)
                    )}>
                    <img className="imageLoveur" src={character.image} alt={character.name} />
